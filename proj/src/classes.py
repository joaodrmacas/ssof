...
class FlowGraph:
    ...
    def __str__(self):
        ...
        for source in self.sources:
            graph_str.append(
                f"  {source.identifier} (line {source.line_num})"
            )
        
        graph_str.append("\nSinks:")
        for sink in self.sinks:
            graph_str.append(
                f"  {sink.identifier} (line {sink.line_num})"
            )
    ...
    for pattern in patterns:
        ...
        for source_node in graph.sources:
            for sink_node in graph.sinks:
                ...
                if (source_str in pattern["sources"] and 
                    sink_str in pattern["sinks"]):
                    ...
                    if paths:
                        ...
                        vulnerabilities.append({
                            ...
                            "implicit_flows": "yes" if graph.has_implicit_flow(path) else "no",
